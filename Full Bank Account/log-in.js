...
btn1.addEventListener('click', function (e) {
    ...
    if (inp4.length === 0) {
        l4.textContent = 'შეიყვანეთ პაროლი!'
    } else {
        l4.textContent = '';
    }
    let array = JSON.parse(localStorage.getItem('arrayy')) || [];
    if (!array.includes(inp3)) {
        alert('ესეთი მეილი არარსებობს გთხოვთ დარეგისტრირდეთ!');
        return;
    } else {
        localStorage.setItem('namee', inp1)
        ...
        if (inp1.length > 0 && inp2.length > 0 && inp3.includes(gmail) && inp4.length > 0) {
            l5.textContent = 'წარმატებით შეხვედით აქაუნთზე!'
            setTimeout(() => {
